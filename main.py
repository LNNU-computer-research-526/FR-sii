...
def main():
    ...
    parser.add_argument('--movie_genre', type=int, default=19, help='')
    args = parser.parse_args()
    print(args)

    dataloader = Dataset(args.dataset, num_his_month=args.num_his_month, num_cur_month=args.num_cur_month, genre=args.movie_genre)
    ...
    lm = FIRE(init_adj_mat, init_time_mat, decay_factor_mat, decay_factor=args.decay_factor, pri_factor=args.pri_factor,
    ...
    for ind, topk in enumerate(eval(args.topks)):
        print('\t- Top{}:\tF1:{:.4f}\tMRR:{:.4f}\tNDCG:{:.4f}\tprecisions:{:.4f}'.format(topk, f1_scores[ind], mrrs[ind], ndcgs[ind],precisions[ind]))
    print('Time info:')
